...
// Determine if a questionnaire answer is ambiguous (context-dependent / "it varies")
function isAmbiguousAnswer(dimension, value) {
  // communication_style "d" is explicitly "It depends on the task"
  if (dimension === 'communication_style' && value === 'd') return true;
  // Other "mixed" or context-dependent ratings
...
function cmdProfileQuestionnaire(options, raw) {
  // If no answers provided, output questions for the caller to present
  if (!options.answers) {
  ...
  // Build analysis JSON matching profiler agent output schema
  const analysis = {
    profile_version: '1.0',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  // Build the full profile section with markers
  const sectionLines = [
    '<!-- GSD:profile-start -->',
    '## Developer Profile',
    '',
...
async function main() {
  ...
  switch (command) {
    ...
    case 'generate-claude-profile': {
      ...
      cmdGenerateClaudeProfile(cwd, { analysis: analysisPath, output: outputPath, global: globalFlag }, raw);
      break;
    }
